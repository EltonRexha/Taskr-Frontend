...
function PasswordReset() {
    ...
    return (
        <>
            {(showCode && !showResetPasswordForm) && (<SecondFactorAuth onComplete={(code) => { handlePasswordReset(code) }} email={email} error={resetCodeError} resend={handlePasswordResetResend} />)}
            {(!showCode && !showResetPasswordForm) && (
                <form className="p-4 lg:p-6 lg:border-2" onSubmit={handleSubmit(async (data) => {
                    await onSubmit(data);
                })} noValidate>
                    <FieldSet>
                        <FieldLegend><p className="text-xl">Provide Your Email</p></FieldLegend>
                        <FieldDescription>
                    ...
                    <Field className="w-full">
                        <Button type="submit" className="cursor-pointer w-full h-12 text-lg">Send Reset Email</Button>
                    </Field>
                </form>
            )}
