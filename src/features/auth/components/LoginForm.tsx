    ...
    <>
      <motion.form
        className={`p-4 lg:p-6 lg:border-2 rounded-sm ${
          (showSecondFactor || showResetPassword) && "hidden"
        }`}
      ...
      >
        <FieldGroup>
          ...
          <div className="flex flex-col gap-1">
            <Link href="/register" className="text-blue-500 w-fit">
              Don&apos;t have an account?
            </Link>
            <button
              type="button"
              className="text-blue-500 w-fit cursor-pointer"
      ...
      {showSecondFactor && (
        <SecondFactorAuth
          onComplete={(code) => {
            handleSecondFactor(code);
          }}
          email={email}
          error={codeError}
          resend={handleSecondFactorResend}
        />
      )}
